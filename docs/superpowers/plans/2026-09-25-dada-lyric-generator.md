...
| 2 | Phrase mode, one lyric file + example song YAML, produces a complete sheet: every section in outline order, every line exactly the template's syllable count | `dada generate examples/pop-song.yaml examples/sample-lyrics.txt --seed 1` exits 0; headers appear in outline order; each line re-tagged by the pronouncer equals its template length | automated: `tests/e2e/test_phrase_mode.py` + narrated: transcript of the run | test_phrase_mode_full_sheet PASSED. Narrated `--seed 1` run exit 0: headers Verse, Pre-Chorus, Chorus, Verse, Pre-Chorus, Chorus, Bridge, Chorus (outline order), no warnings. |
| 3 | Word mode, same inputs, produces a visibly more fragmented sheet, still exact syllable counts | Same command with `--mode word` exits 0, exact per-line counts; mean units per line is higher than phrase mode for the same seed and inputs | automated: `tests/e2e/test_word_mode.py` + narrated: side-by-side transcript | test_word_mode_full_sheet, test_word_mode_is_more_fragmented PASSED. Narrated `--mode word --seed 1` exit 0, e.g. `the something static jacket hold` vs phrase `until the morning finds us warm`. |
...
| 11 | I can copy an example, edit it per the README, and run it | Copy `examples/ballad.yaml`, change the outline and one template following README syntax, run it: exit 0, new structure reflected in output | narrated: transcript of copy, edit, run | Narrated: copied ballad.yaml, added `bridge: [trochaic 3]`, outline -> [verse, chorus, bridge, chorus], verse line 4 -> "x1010"; run exit 0 with `[Bridge]` in output. Note: output repeated one phrase heavily (see final report). |
| 12 | CI runs lint + tests on push/PR and passes | Workflow file runs ruff check, ruff format --check, pytest on push and pull_request; locally, the same steps pass (via `act` if available, otherwise running the exact workflow commands). Full verification requires a GitHub remote and is expected to be user-waived until then | narrated: local run transcript of the workflow steps; GitHub run URL once pushed | Local workflow steps (`act` not installed): `uv sync --locked`, `ruff check` (All checks passed), `ruff format --check` (32 files already formatted), `pytest` (161 passed), exit 0. **user-waived 2026-09-25: GitHub Actions run deferred until the repo has a GitHub remote; reason: no remote yet.** |
